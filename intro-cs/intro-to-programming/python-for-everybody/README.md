<h1 align="center">Python for Everybody</h1>

<h2 align="center">My Achievements</h2>
<h3>Badges</h3>
<img src="./images/enroll.png">
<img src="./images/introductory.png">
<img src="./images/coding.png">
<img src="./images/data_structure.png">
<br>
<br>
<h2 align="center">Solution to the assignments</h2>
<table border="1" align="center">
    <tr>
        <th>Chapter No.</th>
        <th>Name</th>
        <th>Assignments</th>
    </tr>
    <tr>
        <td rowspan="2">0</td>
        <td rowspan="2">Installing Python</td>
        <td><a href="./chapter0/code.png">Code</a>
    </tr>
    <tr>
        <td><a href="./chapter0/terminal.png">Terminal</a></td>
    </tr>
    <tr>
        <td>1</td>
        <td>Why Program?</td>
        <td><a href="./chapter1/exercise-1.1.py">Exercise-1.1</a></td>
    </tr>
    <tr>
        <td rowspan="3">2</td>
        <td rowspan="3">Variables, expressions and Statements</td>
        <td><a href="./chapter2/exercise-2.2.py">Exercise-2.2</a></td>
    </tr>
    <tr>
        <td><a href="./chapter2/exercise-2.3.py">Exercise-2.3</a></td>
    </tr>
    <tr>
        <td><a href="./chapter2/exercise-2.5.py">Exercise-2.5</a></td>
    </tr>
    <tr>
        <td rowspan="3">3</td>
        <td rowspan="3">Conditional Executions</td>
        <td><a href="./chapter3/exercise-3.1.py">Exercise-3.1</a></td>
    </tr>
    <tr>
        <td><a href="./chapter3/exercise-3.2.py">Exercise-3.2</a></td>
    </tr>
    <tr>
        <td><a href="./chapter3/exercise-3.3.py">Exercise-3.3</a></td>
    </tr>
    <tr>
        <td rowspan="4">4</td>
        <td rowspan="4">Functions</td>
        <td><a href="./chapter4/exercise-4.1.py">Exercise-4.1</a></td>
    </tr>
    <tr>
        <td><a href="./chapter4/exercise-4.2-4.3.py">Exercise-4.2 and 4.3</a></td>
    </tr>
    <tr>
        <td><a href="./chapter4/exercise-4.6.py">Exercise-4.6</a></td>
    </tr>
    <tr>
        <td><a href="./chapter4/exercise-4.7.py">Exercise-4.7</a></td>
    </tr>
    <tr>
        <td rowspan="2">5</td>
        <td rowspan="2">Iterations</td>
        <td><a href="./chapter5/exercise-5.1.py">Exercise-5.1</a></td>
    </tr>
    <tr>
        <td><a href="./chapter5/exercise-5.2.py">Exercise-5.2</a></td>
    </tr>
    <tr>
        <td rowspan="3">6</td>
        <td rowspan="3">Strings</td>
        <td><a href="./chapter6/exercise-6.1.py">Exercise-6.1</a></td>
    </tr>
    <tr>
        <td><a href="./chapter6/exercise-6.3.py">Exercise-6.3</a></td>
    </tr>
    <tr>
        <td><a href="./chapter6/exercise-6.5.py">Exercise-6.5</a></td>
    </tr>
    <tr>
        <td rowspan="3">7</td>
        <td rowspan="3">Files</td>
        <td><a href="./chapter7/exercise-7.1.py">Exercise-7.1</a></td>
    </tr>
    <tr>
        <td><a bref="./chapter7/exercise-7.2.py">Exercise-7.2</a></td>
    </tr>
    <tr>
        <td><a href="./chapter7/exercise-7.3.py">Exercise-7.3</a></td>
    </tr>
    <tr>
        <td rowspan="6">8</td>
        <td rowspan="6">Lists</td>
        <td><a href="./chapter8/exercise-8.1.py">Exercise-8.1</a></td>
    </tr>
    <tr>
        <td><a href="./chapter8/exercise-8.2.py">Exercise-8.2</a></td>
    </tr>
    <tr>
        <td><a href="./chapter8/exercise-8.3.py">Exercise-8.3</a></td>
    </tr>
    <tr>
        <td><a href="./chapter8/exercise-8.4.py">Exercise-8.4</a></td>
    </tr>
    <tr>
        <td><a href="./chapter8/exercise-8.5.py">Exercise-8.5</a></td>
    </tr>
    <tr>
        <td><a href="./chapter8/exercise-8.6.py">Exercise-8.6</a></td>
    </tr>
    <tr>
        <td rowspan="5">9</td>
        <td rowspan="5">Dictionaries</td>
        <td><a href="./chapter9/exercise-9.1.py">Exercise-9.1</a></td>
    </tr>
    <tr>
        <td><a href="./chapter9/exercise-9.2.py">Exercise-9.2</a></td>
    </tr>
    <tr>
        <td><a href="./chapter9/exercise-9.3.py">Exercise-9.3</a></td>
    </tr>
    <tr>
        <td><a href="./chapter9/exercise-9.4.py">Exercise-9.4</a></td>
    </tr>
    <tr>
        <td><a href="./chapter9/exercise-9.5.py">Exercise-9.5</a></td>
    </td>
    <tr>
        <td rowspan="2">10</td>
        <td rowspan="2">Tuples</td>
        <td><a href="./chapter10/exercise-10.1.py">Exercise-10.1</a></td>
    </tr>
    <tr>
        <td><a href="./chapter10/exercise-10.2.py">Exercise-10.2</a></td>
    </tr>
</table>